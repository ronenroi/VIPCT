...
def main(cfg: DictConfig):
    ...
    relative_mass_err = []
    batch_time_net = []
    val_image = sio.loadmat('/wdata/yaelsc/Data/CASS_50m_256x256x139_600CCN/pushbroom/ROI/rebatel_iccp22/18S8E_experiment/croped_airmspi_9images_for_Roi.mat')['croped_airmspi_images'] # 18S8E_experiment

    if cfg.data.n_cam != 9:
        val_image = np.delete(val_image, cfg.data.drop_index, 0)
    val_image = torch.tensor(val_image,device=device).float()[None]

    masks = sio.loadmat('/wdata/yaelsc/Data/CASS_50m_256x256x139_600CCN/pushbroom/ROI/rebatel_iccp22/18S8E_experiment/mask_52x52x32_vox50x50x40m.mat')['mask'] # 18S8E_experiment


    # mapping_path = '/wdata/yaelsc/Data/CASS_50m_256x256x139_600CCN/pushbroom/ROI/rebatel_iccp22/18S8E_experiment/voxel_pixel_list52x52x32_BOMEX_img350x350.pkl' # 18S8E_experiment
    # with open(mapping_path, 'rb') as f:
    #     mapping = pickle.load(f)
    # # images_mapping_list = sio.loadmat(mapping_path)['map']
    # pixel_center_path = '/wdata/roironen/Data/shdom_measurements_pixels_center.mat'
    ...
    #
    # with open('/wdata/roironen/Data/AirMSPI-Varying/test/rebat_images_mapping_lists32x32x32_18S8E_img350x350.pkl', 'wb') as f:
    #     pickle.dump(images_mapping_list, f, pickle.HIGHEST_PROTOCOL)
    # with open('/wdata/roironen/Data/AirMSPI-Varying/test/rebat_pixel_centers_lists32x32x32_18S8E_img350x350.pkl', 'wb') as f:
    #     pickle.dump(pixel_centers_list, f, pickle.HIGHEST_PROTOCOL)

    with open('/wdata/roironen/Data/AirMSPI-Varying/test/rebat_images_mapping_lists32x32x32_18S8E_img350x350.pkl', 'rb') as f:
        images_mapping_list = pickle.load(f)
    with open('/wdata/roironen/Data/AirMSPI-Varying/test/rebat_pixel_centers_lists32x32x32_18S8E_img350x350.pkl', 'rb') as f:
    ...
    gx = np.linspace(0,0.05 * 52,52, dtype=np.float32)
    gy = np.linspace(0, 0.05 * 52, 52, dtype=np.float32)
